// src/components/Audience/Audience.js
'use client';
...
// Os ícones são elementos JSX e permanecem no componente.
const profileIcons = [
...
const Audience = ({ dictionary }) => {
  ...
  // Se o dicionário não estiver carregado, não renderiza nada.
  if (!dictionary) {
    return null;
  }
    ...
    <section id="audience" className={styles.audienceSection} ref={sectionRef}>
      <div className={`container ${styles.container}`}>
        ...
        <div className={styles.grid} ref={gridRef}>
          {/* Mapeia os dados do dicionário em vez de usar a constante interna */}
          {dictionary.profiles.map((profile, index) => (
            <div key={index} className={styles.card}>
              <div className={styles.cardContent}>
                {/* Combina o ícone do array local com o texto do dicionário */}
